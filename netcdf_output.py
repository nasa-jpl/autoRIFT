...
def netCDF_packaging_intermediate(
    Dx,
    Dy,
    InterpMask,
    ChipSizeX,
    ...
    origSize,
    noDataMask,
    filename='./autoRIFT_intermediate.nc',
):
...
def netCDF_packaging(
    ...
    if pair_type == 'radar':
        ...
        alpha_sp = (DX * scale_factor_1) / (
            offset2vy_2 / (offset2vx_1 * offset2vy_2 - offset2vx_2 * offset2vy_1) * (-SX)
            - offset2vx_2 / (offset2vx_1 * offset2vy_2 - offset2vx_2 * offset2vy_1) * (-SY)
        )
        alpha_ref = (DX * scale_factor_1) / (
    ...
    if srs.GetAttrValue('PROJECTION') == 'Polar_Stereographic':
        ...
        var.setncattr(
            'latitude_of_projection_origin', np.sign(srs.GetProjParm('latitude_of_origin')) * 90.0
        ...
        var.setncattr(
            'GeoTransform', ' '.join(str(x) for x in tran)
    ...
    if pair_type == 'radar':
        vx_error_mod = (
            (error_vector[0][0] * IMG_INFO_DICT['date_dt'] + error_vector[1][0]) / IMG_INFO_DICT['date_dt'] * 365
        )
    else:
    ...
    var.setncattr(
        'stable_shift_flag_description',
        'flag for applying velocity bias correction: 0 = no correction; '
    ...
    if pair_type == 'radar':
        vy_error_mod = (
            (error_vector[0][1] * IMG_INFO_DICT['date_dt'] + error_vector[1][1]) / IMG_INFO_DICT['date_dt'] * 365
        )
    else:
    ...
    var.setncattr(
        'error_stationary_description',
        'RMSE over stable surfaces, stationary or slow-flowing surfaces '
    ...
    if pair_type == 'radar':
        ...
        vr_error_mod = (
            (error_vector[0][2] * IMG_INFO_DICT['date_dt'] + error_vector[1][2]) / IMG_INFO_DICT['date_dt'] * 365
        )
        if stable_shift_applied == 1:
        ...
        va_error_mod = (
            (error_vector[0][3] * IMG_INFO_DICT['date_dt'] + error_vector[1][3]) / IMG_INFO_DICT['date_dt'] * 365
        )
        if stable_shift_applied == 1:
        ...
        vxp_error_mod = (
            (error_vector[0][4] * IMG_INFO_DICT['date_dt'] + error_vector[1][4]) / IMG_INFO_DICT['date_dt'] * 365
        )
        vyp_error_mod = (
            (error_vector[0][5] * IMG_INFO_DICT['date_dt'] + error_vector[1][5]) / IMG_INFO_DICT['date_dt'] * 365
        )
...
def cal_swath_offset_bias(
    ...
    if (
        (np.nanstd(VX[mask2]) < 25)
        & (np.nanstd(VX[mask1]) < 25)
        & (np.nanstd(VY[mask2]) < 25)
        & (np.nanstd(VY[mask1]) < 25)
    ):
    ...
    if flag21 == 1:
        DX, DY = rotate_vel2radar(
            rngind, azmind, DX, DY, swath_border, swath_border_full, GridSpacingX, ScaleChipSizeY, 0
        )
        print('sharp peaks corrected at subswath 2 and 1 borders')
    if flag32 == 1:
        DX, DY = rotate_vel2radar(
            rngind, azmind, DX, DY, swath_border, swath_border_full, GridSpacingX, ScaleChipSizeY, 1
        )
