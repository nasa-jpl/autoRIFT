...
#        ChipRangeX = ChipRangeX[ChipRangeX < (2**8 - 1)]
        ...
        print(self.ChipSize0X)
        print(ChipSizeUniX)

        # define course filter
        DispFiltC = DISP_FILT()
        #            if ChipSizeUniX[i] == ChipSizeUniX[0]: #ASG ----------------
        DispFiltC.FracValid = self.FracValid*self.GridSize_ChipSize0_Ratio#ASG ----------------
        DispFiltC.FracSearch = self.FracSearch #ASG ----------------
        DispFiltC.FiltWidth = int(((self.FiltWidth+1)*self.GridSize_ChipSize0_Ratio) - 1) #ASG ----------------
        DispFiltC.Iter = int(self.Iter -1) #ASG ----------------
        DispFiltC.MadScalar = int(self.MadScalar) #ASG ----------------
        ...
        for i in range(ChipSizeUniX.__len__()):
            ...
            if self.ChipSize0X != (ChipSizeUniX[i]*self.GridSize_ChipSize0_Ratio): #ASG
                Scale = self.ChipSize0X / (ChipSizeUniX[i]*self.GridSize_ChipSize0_Ratio) #ASG
...
#                SearchLimitY0[np.logical_not(M0)] = 0
            ...
            if ((xGrid0.shape[0] - 2)/(self.sparseSearchSampleRate/self.GridSize_ChipSize0_Ratio) < 5) | ((xGrid0.shape[1] - 2)/(self.sparseSearchSampleRate/self.GridSize_ChipSize0_Ratio) < 5): #ASG ----------------
                Flag = 2
                return Flag

            # Setup for coarse search: sparse sampling / resize
            rIdxC = slice((self.sparseSearchSampleRate/self.GridSize_ChipSize0_Ratio)-1,xGrid0.shape[0],(self.sparseSearchSampleRate/self.GridSize_ChipSize0_Ratio)) #ASG ----------------
            cIdxC = slice((self.sparseSearchSampleRate/self.GridSize_ChipSize0_Ratio)-1,xGrid0.shape[1],(self.sparseSearchSampleRate/self.GridSize_ChipSize0_Ratio)) #ASG ----------------
            xGrid0C = xGrid0[rIdxC,cIdxC]
            yGrid0C = yGrid0[rIdxC,cIdxC]

#            pdb.set_trace()

            if np.remainder((self.sparseSearchSampleRate/self.GridSize_ChipSize0_Ratio),2) == 0:  #ASG ----------------
                filtWidth = (self.sparseSearchSampleRate/self.GridSize_ChipSize0_Ratio) + 1  #ASG ----------------
            else:
                filtWidth = (self.sparseSearchSampleRate/self.GridSize_ChipSize0_Ratio)  #ASG ----------------
...
#            pdb.set_trace()
            ...
            dstShape = (int(MC2.shape[0]*(self.sparseSearchSampleRate/self.GridSize_ChipSize0_Ratio)),int(MC2.shape[1]*(self.sparseSearchSampleRate/self.GridSize_ChipSize0_Ratio))) #ASG ----------------
...
#                pdb.set_trace()
                Scale = (ChipSizeUniX[i]*self.GridSize_ChipSize0_Ratio) / self.ChipSize0X  #ASG
...
#        pdb.set_trace()
    ...
    def __init__(self):
        ...
        self.DataType = 0
        self.GridSize_ChipSize0_Ratio = 0.5;  #ASG
