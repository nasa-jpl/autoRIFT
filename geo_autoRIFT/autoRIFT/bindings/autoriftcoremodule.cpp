...
struct autoRiftCore {
  ...
  //  If taking all the variables here in the structure, the complicated computation can be performed in another C++
  //  file,
...
PyMODINIT_FUNC PyInit_autoriftcore() {
...
PyObject *arPixDisp_u(PyObject *self, PyObject *args) {
  ...
  cv::Mat search_limit_x =
      cv::Mat(cv::Size(widX, lenX), CV_32FC1, reinterpret_cast<float *>(PyArray_DATA(SearchLimitX)));
  cv::Mat search_limit_y =
      cv::Mat(cv::Size(widX, lenX), CV_32FC1, reinterpret_cast<float *>(PyArray_DATA(SearchLimitY)));
