...
double norm_C(isce3::core::Vec3 *a) { return sqrt(dot_C(a, a)); }
...
void cross_C2(isce3::core::Vec3 *a, isce3::core::Vec3 *b, isce3::core::Vec3 *temp) { (*temp) = (*a).cross(*b); }
...
void geoGridRadar::geogridRadar() {
  ...
  std::cout << "Xlimits : " << geoTrans[0] + pOff * geoTrans[1] << "  " << geoTrans[0] + (pOff + pCount) * geoTrans[1]
            << "\n";
  ...
  while (std::getline(ifidt, line)) {
    if (line.find("<UTC>") != std::string::npos) {
      ...
      if ((t0.secondsSinceEpoch() >= itimed.secondsSinceEpoch()) and
          (t0.secondsSinceEpoch() <= ftimed.secondsSinceEpoch())) {
        ++num;
      }
    }
  }
  ...
  std::string str = "";
  double adfGeoTransform[6] = {geoTrans[0] + pOff * geoTrans[1], geoTrans[1], 0,
                               geoTrans[3] + lOff * geoTrans[5], 0,           geoTrans[5]};
  ...
  if (poDriver == NULL) exit(107);

  str = pixlinename;
  const char *pszDstFilename = str.c_str();
  poDstDS = poDriver->Create(pszDstFilename, pCount, lCount, 2, GDT_Int32, papszOptions);
  ...
  if ((dhdxname != "") & (vxname != "")) {
    ...
    if (poDriverOff == NULL) exit(107);

    str = offsetname;
    const char *pszDstFilenameOff = str.c_str();
    poDstDSOff = poDriverOff->Create(pszDstFilenameOff, pCount, lCount, 2, GDT_Int32, papszOptions);
  ...
  if ((dhdxname != "") & (srxname != "")) {
    ...
    if (poDriverSch == NULL) exit(107);

    str = searchrangename;
    const char *pszDstFilenameSch = str.c_str();
    poDstDSSch = poDriverSch->Create(pszDstFilenameSch, pCount, lCount, 2, GDT_Int32, papszOptions);
  ...
  if (csminxname != "") {
    ...
    if (poDriverMin == NULL) exit(107);

    str = chipsizeminname;
    const char *pszDstFilenameMin = str.c_str();
    poDstDSMin = poDriverMin->Create(pszDstFilenameMin, pCount, lCount, 2, GDT_Int32, papszOptions);
  ...
  if (csmaxxname != "") {
    ...
    if (poDriverMax == NULL) exit(107);

    str = chipsizemaxname;
    const char *pszDstFilenameMax = str.c_str();
    poDstDSMax = poDriverMax->Create(pszDstFilenameMax, pCount, lCount, 2, GDT_Int32, papszOptions);
  ...
  if (ssmname != "") {
    ...
    if (poDriverMsk == NULL) exit(107);

    str = stablesurfacemaskname;
    const char *pszDstFilenameMsk = str.c_str();
    poDstDSMsk = poDriverMsk->Create(pszDstFilenameMsk, pCount, lCount, 1, GDT_Int32, papszOptions);
  ...
  if (dhdxname != "") {
    ...
    if (poDriverRO2VX == NULL) exit(107);

    str = ro2vx_name;
    const char *pszDstFilenameRO2VX = str.c_str();
    poDstDSRO2VX = poDriverRO2VX->Create(pszDstFilenameRO2VX, pCount, lCount, 3, GDT_Float64, papszOptions);
    ...
    if (poDriverRO2VY == NULL) exit(107);

    str = ro2vy_name;
    const char *pszDstFilenameRO2VY = str.c_str();
    poDstDSRO2VY = poDriverRO2VY->Create(pszDstFilenameRO2VY, pCount, lCount, 3, GDT_Float64, papszOptions);
    ...
    if (poDriverSF == NULL) exit(107);

    str = sfname;
    const char *pszDstFilenameSF = str.c_str();
    poDstDSSF = poDriverSF->Create(pszDstFilenameSF, pCount, lCount, 2, GDT_Float64, papszOptions);
  ...
  for (int ii = 0; ii < lCount; ii++) {
    ...
    int status =
        demDS->GetRasterBand(1)->RasterIO(GF_Read, pOff, lOff + ii, pCount, 1, (void *)(demLine.data()), pCount, 1,
                                          GDT_Float64, sizeof(double), sizeof(double) * pCount, NULL);
    ...
    if (dhdxname != "") {
      status = sxDS->GetRasterBand(1)->RasterIO(GF_Read, pOff, lOff + ii, pCount, 1, (void *)(sxLine.data()), pCount, 1,
                                                GDT_Float64, sizeof(double), sizeof(double) * pCount, NULL);
      ...
      status = syDS->GetRasterBand(1)->RasterIO(GF_Read, pOff, lOff + ii, pCount, 1, (void *)(syLine.data()), pCount, 1,
                                                GDT_Float64, sizeof(double), sizeof(double) * pCount, NULL);
    ...
    if (vxname != "") {
      status = vxDS->GetRasterBand(1)->RasterIO(GF_Read, pOff, lOff + ii, pCount, 1, (void *)(vxLine.data()), pCount, 1,
                                                GDT_Float64, sizeof(double), sizeof(double) * pCount, NULL);
      ...
      status = vyDS->GetRasterBand(1)->RasterIO(GF_Read, pOff, lOff + ii, pCount, 1, (void *)(vyLine.data()), pCount, 1,
                                                GDT_Float64, sizeof(double), sizeof(double) * pCount, NULL);
    ...
    if (srxname != "") {
      status = srxDS->GetRasterBand(1)->RasterIO(GF_Read, pOff, lOff + ii, pCount, 1, (void *)(srxLine.data()), pCount,
                                                 1, GDT_Float64, sizeof(double), sizeof(double) * pCount, NULL);
      ...
      status = sryDS->GetRasterBand(1)->RasterIO(GF_Read, pOff, lOff + ii, pCount, 1, (void *)(sryLine.data()), pCount,
                                                 1, GDT_Float64, sizeof(double), sizeof(double) * pCount, NULL);
    ...
    if (csminxname != "") {
      status =
          csminxDS->GetRasterBand(1)->RasterIO(GF_Read, pOff, lOff + ii, pCount, 1, (void *)(csminxLine.data()), pCount,
                                               1, GDT_Float64, sizeof(double), sizeof(double) * pCount, NULL);
      ...
      status =
          csminyDS->GetRasterBand(1)->RasterIO(GF_Read, pOff, lOff + ii, pCount, 1, (void *)(csminyLine.data()), pCount,
                                               1, GDT_Float64, sizeof(double), sizeof(double) * pCount, NULL);
    ...
    if (csmaxxname != "") {
      status =
          csmaxxDS->GetRasterBand(1)->RasterIO(GF_Read, pOff, lOff + ii, pCount, 1, (void *)(csmaxxLine.data()), pCount,
                                               1, GDT_Float64, sizeof(double), sizeof(double) * pCount, NULL);
      ...
      status =
          csmaxyDS->GetRasterBand(1)->RasterIO(GF_Read, pOff, lOff + ii, pCount, 1, (void *)(csmaxyLine.data()), pCount,
                                               1, GDT_Float64, sizeof(double), sizeof(double) * pCount, NULL);
    ...
    if (ssmname != "") {
      status = ssmDS->GetRasterBand(1)->RasterIO(GF_Read, pOff, lOff + ii, pCount, 1, (void *)(ssmLine.data()), pCount,
                                                 1, GDT_Float64, sizeof(double), sizeof(double) * pCount, NULL);
    ...
    for (int jj = 0; jj < pCount; jj++) {
      ...
      if (srxname != "") {
        ...
        schrng1[0] *= std::max(
            max_factor * ((dt_unity - 1) * max_factor + (max_factor - 1) - (max_factor - 1) * dt / 24.0 / 3600.0) /
                ((dt_unity - 1) * max_factor),
            1.0);
      ...
      if ((rgind > nPixels - 1) | (rgind < 1 - 1) | (azind > nLines - 1) | (azind < 1 - 1)) {
        ...
        if (dhdxname != "") {
          ...
          if (cross_check > 1.0) {
            raster1a[jj] = normal[2] / (dt / dr / 365.0 / 24.0 / 3600.0) * (normal[2] * temp[1] - normal[1] * temp[2]) /
                           ((normal[2] * los[0] - normal[0] * los[2]) * (normal[2] * temp[1] - normal[1] * temp[2]) -
                            (normal[2] * temp[0] - normal[0] * temp[2]) * (normal[2] * los[1] - normal[1] * los[2]));
            raster1b[jj] = -normal[2] / (dt / norm_C(&da) / 365.0 / 24.0 / 3600.0) *
                           (normal[2] * los[1] - normal[1] * los[2]) /
                           ((normal[2] * los[0] - normal[0] * los[2]) * (normal[2] * temp[1] - normal[1] * temp[2]) -
                            (normal[2] * temp[0] - normal[0] * temp[2]) * (normal[2] * los[1] - normal[1] * los[2]));
            raster2a[jj] = -normal[2] / (dt / dr / 365.0 / 24.0 / 3600.0) *
                           (normal[2] * temp[0] - normal[0] * temp[2]) /
                           ((normal[2] * los[0] - normal[0] * los[2]) * (normal[2] * temp[1] - normal[1] * temp[2]) -
                            (normal[2] * temp[0] - normal[0] * temp[2]) * (normal[2] * los[1] - normal[1] * los[2]));
            raster2b[jj] = normal[2] / (dt / norm_C(&da) / 365.0 / 24.0 / 3600.0) *
                           (normal[2] * los[0] - normal[0] * los[2]) /
                           ((normal[2] * los[0] - normal[0] * los[2]) * (normal[2] * temp[1] - normal[1] * temp[2]) -
                            (normal[2] * temp[0] - normal[0] * temp[2]) * (normal[2] * los[1] - normal[1] * los[2]));
          } else {
            raster1a[jj] = nodata_out;
          ...
          if (srxname != "") {
            if ((schrng1[0] == nodata) | (schrng1[0] == 0)) {
              ...
              sr_raster22[jj] =
                  std::abs(std::round(dot_C(&schrng1v, &temp) * dt / norm_C(&alt) / 365.0 / 24.0 / 3600.0 * 1));
              if (std::abs(std::round(dot_C(&schrng2v, &los) * dt / dr / 365.0 / 24.0 / 3600.0 * 1)) >
                  sr_raster11[jj]) {
                sr_raster11[jj] = std::abs(std::round(dot_C(&schrng2v, &los) * dt / dr / 365.0 / 24.0 / 3600.0 * 1));
              }
              if (std::abs(std::round(dot_C(&schrng2v, &temp) * dt / norm_C(&alt) / 365.0 / 24.0 / 3600.0 * 1)) >
                  sr_raster22[jj]) {
                sr_raster22[jj] =
                    std::abs(std::round(dot_C(&schrng2v, &temp) * dt / norm_C(&alt) / 365.0 / 24.0 / 3600.0 * 1));
              }
              if (sr_raster11[jj] == 0) {
                sr_raster11[jj] = 1;
              }
    ...
    poBand1->RasterIO(GF_Write, 0, ii, pCount, 1, raster1, pCount, 1, GDT_Int32, 0, 0);
    poBand2->RasterIO(GF_Write, 0, ii, pCount, 1, raster2, pCount, 1, GDT_Int32, 0, 0);

    if ((dhdxname != "") & (vxname != "")) {
      poBand1Off->RasterIO(GF_Write, 0, ii, pCount, 1, raster11, pCount, 1, GDT_Int32, 0, 0);
      poBand2Off->RasterIO(GF_Write, 0, ii, pCount, 1, raster22, pCount, 1, GDT_Int32, 0, 0);
    }

    if ((dhdxname != "") & (srxname != "")) {
      poBand1Sch->RasterIO(GF_Write, 0, ii, pCount, 1, sr_raster11, pCount, 1, GDT_Int32, 0, 0);
      poBand2Sch->RasterIO(GF_Write, 0, ii, pCount, 1, sr_raster22, pCount, 1, GDT_Int32, 0, 0);
    }

    if (csminxname != "") {
      poBand1Min->RasterIO(GF_Write, 0, ii, pCount, 1, csmin_raster11, pCount, 1, GDT_Int32, 0, 0);
      poBand2Min->RasterIO(GF_Write, 0, ii, pCount, 1, csmin_raster22, pCount, 1, GDT_Int32, 0, 0);
    }

    if (csmaxxname != "") {
      poBand1Max->RasterIO(GF_Write, 0, ii, pCount, 1, csmax_raster11, pCount, 1, GDT_Int32, 0, 0);
      poBand2Max->RasterIO(GF_Write, 0, ii, pCount, 1, csmax_raster22, pCount, 1, GDT_Int32, 0, 0);
    }

    if (ssmname != "") {
      poBand1Msk->RasterIO(GF_Write, 0, ii, pCount, 1, ssm_raster, pCount, 1, GDT_Int32, 0, 0);
    }

    if (dhdxname != "") {
      poBand1RO2VX->RasterIO(GF_Write, 0, ii, pCount, 1, raster1a, pCount, 1, GDT_Float64, 0, 0);
      poBand2RO2VX->RasterIO(GF_Write, 0, ii, pCount, 1, raster1b, pCount, 1, GDT_Float64, 0, 0);
      poBand3RO2VX->RasterIO(GF_Write, 0, ii, pCount, 1, raster1c, pCount, 1, GDT_Float64, 0, 0);
      poBand1RO2VY->RasterIO(GF_Write, 0, ii, pCount, 1, raster2a, pCount, 1, GDT_Float64, 0, 0);
      poBand2RO2VY->RasterIO(GF_Write, 0, ii, pCount, 1, raster2b, pCount, 1, GDT_Float64, 0, 0);
      poBand3RO2VY->RasterIO(GF_Write, 0, ii, pCount, 1, raster2c, pCount, 1, GDT_Float64, 0, 0);
      poBand1SF->RasterIO(GF_Write, 0, ii, pCount, 1, sf_raster1, pCount, 1, GDT_Float64, 0, 0);
      poBand2SF->RasterIO(GF_Write, 0, ii, pCount, 1, sf_raster2, pCount, 1, GDT_Float64, 0, 0);
    }
  }
