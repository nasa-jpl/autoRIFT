...
void geoGridOptical::geogridOptical() {
  ...
  if (dhdxname != "") {
    ...
    poDstDSRO2VX = poDriverRO2VX->Create(pszDstFilenameRO2VX, pCount, lCount, 2, GDT_Float64, papszOptions);
    ...
    poDstDSRO2VY = poDriverRO2VY->Create(pszDstFilenameRO2VY, pCount, lCount, 2, GDT_Float64, papszOptions);
    ...
    for (int jj = 0; jj < pCount; jj++) {
      ...
      if ((xind > nPixels - 1) | (xind < 1 - 1) | (yind > nLines - 1) | (yind < 1 - 1)) {
        ...
        if (dhdxname != "") {
          ...
          if (cross_check > 1.0) {
            raster1a[jj] =
                normal[2] / (dt / XSize / 365.0 / 24.0 / 3600.0) * (normal[2] * yunit[1] - normal[1] * yunit[2]) /
                ((normal[2] * xunit[0] - normal[0] * xunit[2]) * (normal[2] * yunit[1] - normal[1] * yunit[2]) -
                 (normal[2] * yunit[0] - normal[0] * yunit[2]) * (normal[2] * xunit[1] - normal[1] * xunit[2]));
            raster1b[jj] =
                -normal[2] / (dt / YSize / 365.0 / 24.0 / 3600.0) * (normal[2] * xunit[1] - normal[1] * xunit[2]) /
                ((normal[2] * xunit[0] - normal[0] * xunit[2]) * (normal[2] * yunit[1] - normal[1] * yunit[2]) -
                 (normal[2] * yunit[0] - normal[0] * yunit[2]) * (normal[2] * xunit[1] - normal[1] * xunit[2]));
            raster2a[jj] =
                -normal[2] / (dt / XSize / 365.0 / 24.0 / 3600.0) * (normal[2] * yunit[0] - normal[0] * yunit[2]) /
                ((normal[2] * xunit[0] - normal[0] * xunit[2]) * (normal[2] * yunit[1] - normal[1] * yunit[2]) -
                 (normal[2] * yunit[0] - normal[0] * yunit[2]) * (normal[2] * xunit[1] - normal[1] * xunit[2]));
            raster2b[jj] =
                normal[2] / (dt / YSize / 365.0 / 24.0 / 3600.0) * (normal[2] * xunit[0] - normal[0] * xunit[2]) /
                ((normal[2] * xunit[0] - normal[0] * xunit[2]) * (normal[2] * yunit[1] - normal[1] * yunit[2]) -
                 (normal[2] * yunit[0] - normal[0] * yunit[2]) * (normal[2] * xunit[1] - normal[1] * xunit[2]));
          } else {
            raster1a[jj] = nodata_out;
          ...
          if (srxname != "") {
            if ((schrng1[0] == nodata) | (schrng1[0] == 0)) {
              ...
              if (std::abs(std::round(dot_C(schrng2, xunit) * dt / XSize / 365.0 / 24.0 / 3600.0 * 1)) >
                  sr_raster11[jj]) {
                sr_raster11[jj] = std::abs(std::round(dot_C(schrng2, xunit) * dt / XSize / 365.0 / 24.0 / 3600.0 * 1));
              }
              if (std::abs(std::round(dot_C(schrng2, yunit) * dt / YSize / 365.0 / 24.0 / 3600.0 * 1)) >
                  sr_raster22[jj]) {
