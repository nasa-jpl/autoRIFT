...
void geoGrid::geogrid() {
  ...
  for (int ii = 0; ii < lCount; ii++) {
    ...
    for (int jj = 0; jj < pCount; jj++) {
      ...
      double dopfact;
      double height;
      double vhat[3], that[3], chat[3], nhat[3], delta[3], targVec[3], targXYZ[3], diffvec[3], temp[3], satvc[3],
          altc[3];
      double vmag;
      double major, minor;
      double satDist;
        ...
        if (dhdxname != "") {
          ...
          if (srxname != "") {
            if ((schrng1[0] == nodata) | (schrng1[0] == 0)) {
              ...
              sr_raster22[jj] =
                  std::abs(std::round(dot_C(schrng1, temp) * dt / norm_C(alt) / 365.0 / 24.0 / 3600.0 * 1));
              ...
              if (std::abs(std::round(dot_C(schrng2, temp) * dt / norm_C(alt) / 365.0 / 24.0 / 3600.0 * 1)) >
                  sr_raster22[jj]) {
                sr_raster22[jj] =
                    std::abs(std::round(dot_C(schrng2, temp) * dt / norm_C(alt) / 365.0 / 24.0 / 3600.0 * 1));
              }
              if (sr_raster11[jj] == 0) {
                sr_raster11[jj] = 1;
              }
